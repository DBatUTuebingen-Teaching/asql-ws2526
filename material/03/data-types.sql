...
SELECT t.type_name AS "𝖽ata type"
...
SELECT 6.2 :: int;                  -- ➝ 6
SELECT 6.6 :: int;                  -- ➝ 7
SELECT CAST('2025-05-04' AS date);  -- ➝ 2025-05-04 (May the Force ...)
...
-- Should the cast fail, return NULL (no runtime error)
SELECT TRY_CAST('{"x":42, "ys":[4;2], "z":"fortytwo"}' AS json);


-- Implicit cast from text to target during *input conversion*:
.shell cat ~/AdvancedSQL/slides/Week03/live/text-input.csv
...
-- The following two queries "benchmark" the
...
WITH one_billion_rows(x) AS (
  ...
  FROM   generate_series(1, 1_000_000_000) AS t(x)
)
...
--                 ↓        ↓     ↓                         ↓
SELECT (make_date(2025, months.m, 1) - '1 day'::interval)::date AS last_day_of_month
FROM   generate_series(1,12) AS months(m);


-- DuckDB's date/time library is extensive...
--
SELECT last_day(make_date(2025, months.m, 1)) AS last_day_of_month
FROM   generate_series(1,12) AS months(m);
...
-- ⚠️ Comparisons of text vs. episode values may diverge
--
SELECT least('TPM', 'AOTC');
SELECT least('TPM' :: episode, 'AOTC' :: episode);
...
-- (1) Navigating a JSON value using binary operator ->
...
-- (2) Navigating a JSON value using JSONPath syntax
...
SELECT json_extract(json('{ "a":0, "b": { "c":[1,2], "d":3 } }'),
                    ...
                    -- '$.b.**'                -- all nodes in subtree below b
                    ) AS j;
...
-- (1) Cast JSON object o into row value t
...
-- (2) Turn row values t into JSON objects o
SELECT json(t) AS o
FROM   T AS t;
...
-- ⚠️ Resulting JSON object has duplicate fields
SELECT json_group_object(t.b, t.c) AS o
FROM   T AS t;

-- construct unique field names (text || int casts rhs argument to text)
...
DROP SEQUENCE IF EXISTS T_keys;
CREATE SEQUENCE T_keys START 100 INCREMENT 10;

DROP TABLE IF EXISTS auto_T;
CREATE TABLE auto_T (k int DEFAULT nextval('T_keys') PRIMARY KEY,
                     a int ,
                     b text,
                     c boolean,
                     d int);
