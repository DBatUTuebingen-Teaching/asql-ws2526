...
SELECT t.i
FROM   range('2025-11-27 10:15:00' :: timestamp,
             '2025-11-27 11:45:00' :: timestamp,
...
LATERAL (SELECT d.*
         ...
         LIMIT 3) AS tallest
ORDER BY locomotion.legs, tallest.height;

-- Aside: Finding the *single* tallest two- or four-legged dinosaur is a job for arg_max:
--
SELECT d.legs, arg_max(d.species, d.height)
FROM   dinosaurs AS d
WHERE  d.legs IS NOT NULL
GROUP BY d.legs;

-- Aside² (DuckDB's "friendly SQL"): arg_max comes with an optional third integer
-- parameter n that can aggregate the n tallest dinosaurs, yielding lists of length n:
--
SELECT d.legs, arg_max(d.species, d.height, 3)
FROM   dinosaurs AS d
WHERE  d.legs IS NOT NULL
GROUP BY d.legs;
