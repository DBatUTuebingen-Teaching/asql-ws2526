...
--
--     􀔌¹     􀍬ᵃ           􀔌⁶     􀍬ᵍ           􀔌¹ 􀓭³     􀍬ᵃ ╷ᵈ
--   ²􀔢 􀔓⁵  ᵇ􀎂 􀍳ᶜ        ⁴􀔢 􀔃⁷  ᵇ􀎂 􀍣ᶜ         􀔄 􀔃 􀓮     􀍤 􀍣 ╵
--   􀔄 􀔃􀓮   􀍤 􀍣╵        ¹􀔢 􀔃⁵  ᵈ􀎂 􀍣ᵉ          ² ⁴ ⁵     ᵇ ᶜ ᵉ
--   ³ ⁴⁶   ᵈ ᵉᶠ        􀔄 􀔃    􀍤 􀍣
--                     ² ³    ᶠ ᵃ
...
-- Use one(!) of the following INSERT statements to populate polygon
...
INSERT INTO polygon(i,x,y)
  ...
  FROM   generate_series(1,50_000_000) AS _(i);
...
SELECT list_sum(
         list_transform(
           ...
           p -> p[1].x * p[2].y - p[2].x * p[1].y)) / 2	AS area
...
SELECT t.*
FROM   unnest(['x₁','x₂','x₃']) WITH ORDINALITY AS t(elem,idx);
...
FROM   unnest(string_split('Luke, I am Your Father', ' '))
       WITH ORDINALITY AS words(word, pos);

-- Split string into whitespace-separated words
--
SELECT words.*
FROM   unnest(regexp_split_to_array('Luke, I am Your Father', '\s+'))
       WITH ORDINALITY AS words(word,pos);
--                                                               ↑
...
FROM   Trees AS t,
       unnest(t.parents) WITH ORDINALITY AS node(parent,idx),
       unnest(t.labels)  WITH ORDINALITY AS label(lbl,idx)
...
-- A named RGB color (struct nesting depth: 2)
...
--           ↑
--      ⚠️ cast from SQL list of structs to JSON array of objects
FROM   unnest(json('[{"color": "lime",   "rgb": {"r":153, "g":255, "b":251} },
                     {"color": "barbie", "rgb": {"r":255, "g":102, "b":255} },
                     {"color": "black",  "rgb": {"r":0,   "g":0,   "b":0  } }
                    ]') :: color[]) AS t(c);
--                      ↑
--                 ⚠️ cast from JSON array of object to SQL list of color structs
--
...
-- (1) Map DNS top-level domains to their respective country flags (buggy)
...
-- (2) Use map_concat to fix/update the tld→flag mapping
WITH countries(flags) AS (
  VALUES (map(['de','pl','gb'], ['🇩🇪','🇮🇩','🇬🇧']))
)
