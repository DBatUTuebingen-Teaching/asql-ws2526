...
CREATE OR REPLACE TABLE dinosaurs (
  species text,
  height  float,
  length  float,
  legs   int);
...
SELECT d.species, d.height, d.length,
       (SELECT arg_min(b.legs, abs(b.shape - d.height / d.length))   -- returns legs for body shape that has minimal
        FROM   bodies AS b) AS legs                                  -- distance to d's height to length ratio
FROM  dinosaurs AS d
