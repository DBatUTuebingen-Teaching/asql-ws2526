...
CREATE OR REPLACE TYPE τ AS row(a int, b text, c boolean, d int);
...
SELECT sum(t.d) FILTER (t.c) AS picky,
...
-- Simple pivoting
SELECT sum(t.d) FILTER (t.b = 'x')            AS "∑d in region x",
       sum(t.d) FILTER (t.b = 'y')            AS "∑d in region y",
       sum(t.d) FILTER (t.b NOT IN ('x','y')) AS "∑d elsewhere"
FROM   T As t;
...
CREATE OR REPLACE TABLE prehistoric (
  class        text,
  "herbivore?" boolean,
  legs         int,
  species      text);
