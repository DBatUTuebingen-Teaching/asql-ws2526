...
CREATE OR REPLACE MACRO input() AS '40 + 1 × 2';
...
tokens(tok, i) AS (
  SELECT t.tok, t.i
  FROM   unnest(regexp_split_to_array(input(), '\s+'))
         WITH ORDINALITY AS t(tok, i)
),
...
SELECT input() AS input,
       (SELECT COALESCE(bool_or(g.start), false)
        FROM   grammar g,
               parse AS p
        WHERE  p."from" = 1 AND p."to" = (SELECT count(*) FROM tokens)
