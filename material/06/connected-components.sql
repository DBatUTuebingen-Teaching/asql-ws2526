...
CREATE TEMPORARY TABLE graph AS
  TABLE edges
    UNION
  SELECT e."to" AS "from", e."from" AS "to"  -- flip edge e
  FROM   edges e;
...
--
sane_components(node, component) AS (
...
-- ORDER BY node;
--
-- TABLE sane_components
-- ORDER BY node;
--
-- TABLE subgraphs
-- ORDER BY component, "from", "to";  -- ORDER BY ALL
